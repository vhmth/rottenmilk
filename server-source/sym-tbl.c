...
/* $Header: /usr/local/cvsroot/spimbot/sym-tbl.c,v 1.6 2004/09/16 23:31:57 craig Exp $
*/
	  ...
	  || SYMBOL_IS_DEFINED (EXPR (inst)->symbol))
	{
	  ...
	  if (opcode_is_branch (OPCODE (inst)))
	      ...
	      if (value & 0x8000)
		{
		  ...
		  READ_MEM_INST (current_image, prev_inst, pc - BYTES_PER_WORD);
		  if (prev_inst != NULL
	  ...
	  if ((value & ~field_mask) != 0 && 
			((unsigned)(value & ~field_mask)) != 0xffff0000)
	    {
