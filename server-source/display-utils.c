/* SPIM S20 MIPS simulator.
  ...
  $Header: /usr/local/cvsroot/spimbot/display-utils.c,v 1.4 2004/02/04 05:05:11 craig Exp $
*/
...
#endif
{
  ...
  *buf = '\0';
  ...
  sprintf (bufp, " PC     = %08x    ", reg_image.PC); bufp += strlen (bufp);
  sprintf (bufp, " EPC    = %08x    ", EPC(reg_image)); bufp += strlen (bufp);
  sprintf (bufp, " Cause  = %08x    ", Cause(reg_image)); bufp += strlen (bufp);
  sprintf (bufp, " BadVAddr= %08x\n", BadVAddr(reg_image)); bufp += strlen (bufp);
  sprintf (bufp, " Status = %08x    ", Status_Reg(reg_image)); bufp += strlen (bufp);
  sprintf (bufp, " HI     = %08x    ", reg_image.HI); bufp += strlen (bufp);
  sprintf (bufp, " LO     = %08x\n", reg_image.LO); bufp += strlen (bufp);
  ...
  for (i = 0; i < 8; i++)
  {
    sprintf (bufp, grstr, i, reg_names[i], reg_image.R[i]);
    bufp += strlen (bufp);
    sprintf (bufp, grfill); bufp += strlen (bufp);
    sprintf (bufp, grstr, i+8, reg_names[i+8], reg_image.R[i+8]);
    bufp += strlen (bufp);
    sprintf (bufp, grfill); bufp += strlen (bufp);
    sprintf (bufp, grstr, i+16, reg_names[i+16], reg_image.R[i+16]);
    bufp += strlen (bufp);
    sprintf (bufp, grfill); bufp += strlen (bufp);
    sprintf (bufp, grstr, i+24, reg_names[i+24], reg_image.R[i+24]);
    bufp += strlen (bufp);
    sprintf (bufp, "\n");
    bufp += 1;
  }
  ...
  if (print_fpr_hex)
    ...
    {
      sprintf (bufp, fpstr, 2*i, reg_image.FPR[i]);
      bufp += strlen (bufp);
      sprintf (bufp, fpfill); bufp += strlen (bufp);
      sprintf (bufp, fpstr, 2*i+8, reg_image.FPR[i+4]);
      bufp += strlen (bufp);
      sprintf (bufp, fpfill); bufp += strlen (bufp);
      sprintf (bufp, fpstr, 2*i+16, reg_image.FPR[i+8]);
      bufp += strlen (bufp);
      sprintf (bufp, fpfill); bufp += strlen (bufp);
      sprintf (bufp, fpstr, 2*i+24, reg_image.FPR[i+12]);
      bufp += strlen (bufp);
      sprintf (bufp, "\n");
      bufp += 1;
    }
    ...
    if (print_fpr_hex)
      ...
      /* Use pointers to cast to ints without invoking float->int conversion
	so we can just print the bits. */
	sprintf (bufp, fpstr, i, *(int *)&reg_image.FGR[i]); bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	
	sprintf (bufp, fpstr, i+8, *(int *)&reg_image.FGR[i+8]); bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	
	sprintf (bufp, fpstr, i+16, *(int *)&reg_image.FGR[i+16]); bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	
	sprintf (bufp, fpstr, i+24, *(int *)&reg_image.FGR[i+24]); bufp += strlen (bufp);
	sprintf (bufp, "\n"); bufp += 1;
      }
      else for (i = 0; i < 8; i += 1)
      {
	sprintf (bufp, fpstr, i, reg_image.FGR[i]);
	bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	sprintf (bufp, fpstr, i+8, reg_image.FGR[i+8]);
	bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	sprintf (bufp, fpstr, i+16, reg_image.FGR[i+16]);
	bufp += strlen (bufp);
	sprintf (bufp, fpfill); bufp += strlen (bufp);
	sprintf (bufp, fpstr, i+24, reg_image.FGR[i+24]);
	bufp += strlen (bufp);
	sprintf (bufp, "\n");
	bufp += 1;
      }
...
#endif
{
  ...
  for (i = from; i < to; i += 4)
  {
    READ_MEM_INST (/*context*/0, inst, i);
    if (inst != NULL)
    {
...
#endif
{
  ...
  *string_len += strlen (&buf[*string_len]);
  buf = mem_as_string (DATA_BOT, mem_images[0].data_top, buf, max_buf_len, string_len);

  sprintf (&buf[*string_len], "\n\tSTACK\n");
  *string_len += strlen (&buf[*string_len]);
  buf = mem_as_string (ROUND_DOWN (reg_image.R[29], BYTES_PER_WORD),
		       STACK_TOP - 4096,
		       buf,
		       max_buf_len,
		       string_len);
  ...
  sprintf (&buf[*string_len], "\n\tKERNEL DATA\n");
  *string_len += strlen (&buf[*string_len]);
  buf = mem_as_string (K_DATA_BOT, mem_images[0].k_data_top, buf, max_buf_len, string_len);
...
#endif
{
  ...
  for ( ; i < to; )
  {
    ...
    {
      READ_MEM_WORD (/*context*/0, val, i + j);
      if (val != 0)
	break;
    }
    if (i + j < to)
