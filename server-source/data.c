...
/* $Header: /usr/local/cvsroot/spimbot/data.c,v 1.4 2004/02/04 05:05:11 craig Exp $
*/
...
#define BUMP_DATA_PC(DELTA) {if (reg_images[current_image].in_kernel) \
		reg_images[current_image].next_k_data_pc += DELTA;	 \
				else {reg_images[current_image].next_data_pc += DELTA;}}
...
#endif
{
  reg_images[current_image].in_kernel = 0;
  reg_images[current_image].auto_alignment = 1;
}
...
#ifdef __STDC__
void
data_begins_at_point (int context, mem_addr addr)
#else
void
...
#ifdef __STDC__
void
k_data_begins_at_point (int context, mem_addr addr)
#else
void
...
{
  if (alignment == 0)
    reg_images[current_image].auto_alignment = 0;
...
#endif
{
  for ( ; length > 0; string ++, length --) {
    SET_MEM_BYTE (current_image, DATA_PC, *string);
    BUMP_DATA_PC(1);
  }
  if (null_terminate)
    {
      SET_MEM_BYTE (current_image, DATA_PC, 0);
      BUMP_DATA_PC(1);
    }
}
